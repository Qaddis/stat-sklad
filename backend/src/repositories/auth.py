...
from src.services.auth import get_hash_password, verify_password
...
class UserCRUD:
    ...
    async def edit_user(self, new_data: PatchUser, user_id: str):
        ...
        user_to_edit = await self.get_user_by_id(user_id)
        if verify_password(values["password"], user_to_edit.password):
            real_values = {}
            for value in values:
