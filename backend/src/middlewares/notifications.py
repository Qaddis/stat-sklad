        ...
        for item in supply:
            ...
            if existed_product:
                if existed_product.ingredient.units == UnitsEnum.KILOGRAMS:
                    if (
                        existed_product.quantity - item["quantity"] < KILOGRAM_LIMIT
                    ) and await not_crud.get_notification(
                        existed_product.ingredient_id
                    ):
                        await not_crud.edit_notification(
                            f"Внимание! На складе осталось всего {existed_product.quantity + item["quantity"]} шт. товара {existed_product.ingredient.name}",
                            existed_product.ingredient_id,
                        )
                        await db.commit()
                    elif existed_product.quantity - item["quantity"] < KILOGRAM_LIMIT:
                        notification_obj = Notification(
                            title=f"Истощение запасов: {existed_product.ingredient.name}",
                    ...
                    if (
                        existed_product.quantity - item["quantity"] < PIECES_LIMIT
                    ) and await not_crud.get_notification(
                        existed_product.ingredient_id
                    ):
                        await not_crud.edit_notification(
